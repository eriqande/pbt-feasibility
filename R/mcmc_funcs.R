...
#### Here is a version for fisheries that are 100% electronically sampled ####
# this is here to set values while developing the function below
if(FALSE) {
  r <- dat$recovery
  cs <- dat$catch_sample
  ...
  # just define these so they are in the function...
  reps = 10000
  thin = 10
  unclipped_awts_to_U_minus = TRUE
  no_ad_tag_ceiling = 0.02
...
#' Do the Bayesian mixed fishery proportion estimation. This version for 100% ETD fisheries.
#' 
#' Takes as input the output of squash_data_for_estimation
#' @param  s  The "squashed" list of input data
#' @param unclipped_awts_to_U_minus  For cases when there are few awt fish and you want to assume that
...
cwt_ppn_estimation_function_EDT <- function(s, reps = 10000, thin = 10,
  ...
  if(notbeeps > 0) stop(paste("Bad News.  Should be 100% ETD fishery, but there are", notbeeps, "fish with beep != yes!"))
  ...
  ## and now we sum up the numbers of fish that have cwt and ad-clips
  ## Recall that all of these fish have beeps!
  recovs_factored <- s$recovery
  recovs_factored$ad_clipped <- factor(recovs_factored$ad_clipped, levels = c("yes", "no", "unknown"))
  recovs_factored$cwt_status <- factor(recovs_factored$cwt_status, levels = c("cwt", "no_read", "awt", "no_tag", "unknown"))
  recov_sums <- as.data.frame(table(recovs_factored$ad_clipped, recovs_factored$cwt_status))
  ...
  # of no-adclip fish from the catch-sample information into the no, unknown category
#  recov_sums$n[recov_sums$ad_clipped == "no" & recov_sums$cwt_status == "unknown"] <- 
#    s$catch_sample[s$catch_sample$detection_method == "E", "mr_1st_sample_known_ad_status"] - 
#    s$catch_sample[s$catch_sample$detection_method == "E", "mr_1st_sample_obs_adclips"]
  ...
  ## and for the ones that have unknown values, we will store those numbers in a list.
  tmp <- recov_sums %>% filter((ad_clipped == "unknown" | cwt_status == "unknown") & cwt_status != "no_read")
  beep_counts <- lapply(1:nrow(tmp), function(x) {  # I am naming those "beep_counts" because all the fish that are in it are beep fish
    ret <- list()
    ret$vars <- c(as.character(tmp[x,1]), as.character(tmp[x,2]))
    names(ret$vars) <- names(tmp)[1:2]
    ret$n <- unlist(tmp[x,3])
  ...
  ###########  Right here make the no_beep_counts list which will just
  # have the counts of fish that were no_beeps that can fall into two categories
  # Ad-clip  unknown-cwt   and No-ad-clip unkown-cwt  and   unknown-ad-clip  unknown-cwt  We call them unknown about whether
  # they have a CWT or not, and they get allocated to different categories according to the phi's.
  # Yep, that will work....
  # make some temp vars before transferring to the list
  adc_unk <- unname(unlist((s$catch_sample[1, "mr_2nd_sample_obs_adclips"])))
  no_adc_unk <- unname(unlist((s$catch_sample[1, "mr_2nd_sample_known_ad_status"]))) - adc_unk
  ...
  ### before doing mcmc, we need a function that will take elements of beep_counts, and simulate all the 
  ### individuals in them into the rows of an imputed version of adc_cwt  vp is the vis_count_table.
  ### This thing essentially does the conditioning that needs to get done.
  impute <- function(x, vp) {
    p <- vp$probs
    if(x$vars["ad_clipped"] != "unknown") {
  ...
  #### Now that everything is initialized and we have our convenience variables,
  #### it is time to start doing the iterations of the MCMC.  Before we get started.
  #### Let's create two lists to store the output.  
  ret <- list()
  ret$theta_gs <- vector("list", ceiling(reps / thin)) 
  ...
  for(iterations in 1:reps) {
    ...
    ## Get the beep prob table
    beep_prob_table <- vis_samp_prob_table
    tmp <- beep_prob_table$probs * c(rep(1-beta, 4), alpha, alpha)
    beep_prob_table$probs <- tmp/sum(tmp)
    
    ## Get the no-beep prob table
    no_beep_prob_table <- vis_samp_prob_table
    tmp <- no_beep_prob_table$probs * c(rep(beta, 4), 1 - alpha, 1 - alpha)
    no_beep_prob_table$probs <- tmp/sum(tmp)
    
    
    ### Do the predictive allocations with fish that beeped
    predictive_table <- beep_prob_table
    predictive_table$pred_n <- rmultinom(1, tot_beep_fish, predictive_table$probs)
